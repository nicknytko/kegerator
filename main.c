...
void client(int argc, char **argv, int socketFile, struct sockaddr_un *sock) {
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1], "--addbrew") == 0 && argc == 5) {
        ...
        if (strcmp(argv[1], "--renamebrew") == 0 && argc == 4) {
        ...
        if (strcmp(argv[1], "--listbrews") == 0 && argc == 2) {
            uint32_t numBrews;
            ...
            read(socketFile, brewData,
                 sizeof(struct daemon_ipc_brew_data_t) * numBrews);
...
int main(int argc, char **argv) {
    ...
    if (connect(socketFile, (struct sockaddr *)&socketAddr, sizeof(socketAddr)) ==
        0) {
